...
Two low-level compositing functions.
...
Higher-level Bubble Tea models for dismissible styled notifications. See the
[Notifications](./notifications) page for the full reference.
...
Each type is a `tea.Model` with `Init`, `Update`, `View`, `Done`, and
`PlaceOn` methods. Options are passed as functional options:

```go
n := overlay.NewError(
    overlay.WithTitle("Oops"),
    overlay.WithMessage("Something went wrong."),
    overlay.WithDismissMode(overlay.DismissEither),
    overlay.WithDuration(8 * time.Second),
    overlay.WithKey("esc"),
    overlay.WithPosition(1, 2),
    overlay.WithWidth(50),
    overlay.WithIcon("✗"),
)
```
